...
class KafkaProducer(implicit val mat: ActorMaterializer) extends Actor {
  ...
  def sendMsgToKafka(events: Array[Map[String, Any]])(implicit mat: ActorMaterializer): Future[Done] = {
    ...
    Source.single(
      ProducerMessage.Message(new ProducerRecord[String, String](Configuration.sinkKafkaTopic, write(events)), 100)
...
object KafkaProducer {
  ...
  def props(implicit mat: ActorMaterializer): Props = Props(new KafkaProducer()(mat)).withDispatcher("kafka-message-dispatcher")
}
